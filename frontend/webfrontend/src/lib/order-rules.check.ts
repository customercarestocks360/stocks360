...
assert.equal(
  checkTicket(
    ticket({ assetClass: "forex", label: "EUR/USD", quoteCurrency: "USD", reservePrice: 1.08 }),
  ).error,
  "",
...
assert.equal(
  checkTicket(
    ticket({ assetClass: "stocks", label: "AAPL", quoteCurrency: "USD", reservePrice: 230 }),
  ).error,
  "",
...
assert.equal(
  checkTicket(
    ticket({ assetClass: "stocks", label: "RELIANCE", quoteCurrency: "INR", reservePrice: 3100 }),
  ).error,
  "",
...
assert.equal(
  checkTicket(ticket({ freeMargin: 359 })).needsFunds,
  true,
  "a shortfall is a funds problem",
);
...
assert.equal(
  checkTicket(ticket({ side: "sell", netQuantity: 5, freeQuantity: 5, quantity: 2 })).reserves,
  "units",
);
...
assert.equal(
  checkTicket(ticket({ side: "sell", quantity: 1 })).error,
  "",
  "crypto shorts are allowed",
);
assert.equal(
  checkTicket(
    ticket({
...
assert.equal(
  equitySell.needsFunds,
  false,
  "a borrow this venue cannot arrange is not a funds problem",
);
...
assert.equal(
  checkTicket(ticket({ side: "sell", quantity: 3, netQuantity: 5, freeQuantity: 5 })).error,
  "",
);
assert.match(
...
assert.equal(
  checkTicket(ticket({ side: "sell", quantity: 5, netQuantity: 5, freeQuantity: 5 })).error,
  "",
  "selling exactly the long is a close, not a flip",
);
assert.match(
...
assert.equal(
  checkTicket(ticket({ side: "buy", quantity: 5, netQuantity: -5 })).error,
  "",
  "buying exactly the short closes it",
);
assert.match(
...
assert.equal(
  checkTicket(ticket({ side: "buy", quantity: 10, netQuantity: 5 })).error,
  "",
  "adding to a long is not a flip",
);
assert.equal(
  checkTicket(ticket({ side: "sell", quantity: 10, netQuantity: -5 })).error,
  "",
  "adding to a short is not a flip",
);
...
assert.match(
  checkTicket(ticket({ quantity: 1, reservePrice: 2_000_000, freeMargin: 1e9 })).error,
  /per-order limit/,
);
